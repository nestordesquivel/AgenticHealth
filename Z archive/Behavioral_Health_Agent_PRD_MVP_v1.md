...
A personalized learning engine is explicitly **not required** for V1. Dinner and sleep outcomes may be stored for later analysis.
...
| Resource | Purpose |
| :---- | :---- |
| Vision-capable foundation model API | Convert meal photos into estimated calories and macros. |
| Text/reasoning model API | Convert structured Current State into a concise dinner recommendation if needed. The same multimodal model may handle both to reduce dependencies. |
...
Use the simplest persistence mechanism that preserves the demo workflow.
...
For the hackathon, lightweight local or application-level persistence is sufficient. A production database is not required to validate the concept.

The application should preserve current-day data across normal navigation. Persistence across devices or long-term history is not a P0 requirement.
...
| :---- | :---- |
| **0:00–0:25** | Scaffold one mobile-first web app with Home / Details / Settings and basic navigation. Define the in-memory/local data shape for settings, meals, Current State, and recommendation. |
